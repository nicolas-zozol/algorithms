class MinHeap {
    ...
    subify(i, limit = this.size()) {
        ...
        if (rightIndex <= limit && this.firstIsSmaller(rightValue , leftValue)) {
            smallerChildrenIndex = rightIndex;
        }

        if (this.firstIsSmaller(this.heap[smallerChildrenIndex] , this.heap[i])) {
            this.swap(i, smallerChildrenIndex);
            this.subify(smallerChildrenIndex, limit)
        }
